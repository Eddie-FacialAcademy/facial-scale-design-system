...
```
repo (facial-scale-design-system)
...
└── design-system/                   # PACOTE consumível
    ...
    ├── facial-scale-design-system.css     # DROP-IN: tokens + reset + foco + motion + tipografia + botões + chips/status
    ├── facial-scale-design-tokens.json    # Tokens legíveis por máquina (Framer, Style Dictionary-like, IA)
    ...
    └── copy-deck.facial-scale.json        # Strings de produto do showcase
```

**Duas camadas de classe (importante):**
- **Pacote drop-in** usa classes prefixadas **`fs-*`** (`.fs-btn`, `.fs-fill`, `.fs-h1`, `.fs-status`…). É o que você consome num projeto.
...
3. **Escolha do usuário** persistida em `localStorage['fs-theme']` (`'light'`|`'dark'`), que vence o sistema.
...
### 2.5 Formato do `facial-scale-design-tokens.json`
...
Classe base **`.b`** (drop-in: `.fs-btn`). Composição: `.b` + tamanho (`.sm`/`.md`/`.lg`) + variante (`.fill`/`.solid`/`.outline`/`.ghost`/`.gold`/`.gold-o`). Ícone interno: `.ico` (drop-in `.fs-ico`).
...
.fs-status.is-warning{color:var(--warning);background:var(--warning-bg)}
.fs-status.is-danger {color:var(--danger);background:var(--danger-bg)}
.fs-status.is-info   {color:var(--info);background:var(--info-bg)}
```
...
- **Tema:** init anti-flash + toggle `#themeToggle` (ver 2.1). Chave `localStorage['fs-theme']`.
...
<html lang="pt-BR"><head>
  ...
  <button class="fs-btn fs-fill">Assinar agora</button>
</body></html>
```
